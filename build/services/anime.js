...
export class AnimeService {
    ...
    handleError(error) {
        ...
        if (error instanceof AxiosError) {
            throw new McpError(ErrorCode.InternalError, `Erreur API: ${error.response?.data?.message || error.message}`);
        }
        throw new McpError(ErrorCode.InternalError, `Oops! Erreur: ${String(error)}`);
    }
}
