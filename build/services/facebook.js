...
export class FacebookService {
    ...
    async getPagePosts(limit = 10, after, before, since, until) {
        if (!this.pageId) {
        ...
        try {
            ...
            if (until)
                params.until = until;
            const response = await axios.get(`${FB_API_BASE_URL}/${this.pageId}/posts`, { params });
    ...
    async getPostComments(postId, limit = 25, after) {
        if (!this.accessToken) {
            throw new McpError(ErrorCode.InvalidParams, "Le token d'accès Facebook n'est pas défini");
        }
        try {
            ...
            if (after)
                params.after = after;
            const response = await axios.get(`${FB_API_BASE_URL}/${postId}/comments`, { params });
            return response.data;
        }
        catch (error) {
            this.handleError(error);
    ...
    async postAnime(animeData) {
        ...
        try {
            ...
            const response = await axios.post(`${FB_API_BASE_URL}/${this.pageId}/photos`, formData, {
                headers: {
                    ...formData.getHeaders(),
                },
                params: {
    ...
    async updatePost(postId, message) {
        ...
        try {
            ...
            try {
                const response = await axios.post(`${FB_API_BASE_URL}/${postId}`, { message }, {
                    params: {
                        access_token: this.accessToken,
                    },
                });
                ...
                if (updateError.response?.data?.error?.code === 3 ||
                    ...
                    try {
                        ...
                        const photoResponse = await axios.get(`${FB_API_BASE_URL}/${postId}`, {
                            params: {
                                access_token: this.accessToken,
                        ...
                        if (episodeComments.length > 0) {
                            ...
                            try {
                                ...
                                throw new McpError(ErrorCode.InvalidParams, "Le message de mise à jour doit être au format JSON pour les publications d'anime");
                            }
                            ...
                            throw new McpError(ErrorCode.InternalError, "Cette publication est de type Photo et ne peut pas être mise à jour avec l'API Facebook. " +
                        ...
                        throw new McpError(ErrorCode.InternalError, "Votre application Facebook n'a pas les permissions nécessaires pour mettre à jour les publications. " +
                            "Solution alternative : supprimez la publication existante et créez-en une nouvelle. " +
    ...
    async addEpisodeComment(postId, episode) {
        if (!this.accessToken) {
            throw new McpError(ErrorCode.InvalidParams, "Le token d'accès Facebook n'est pas défini");
        }
        ...
        for (const server of episode.servers) {
            if (server.type === 'facebook') {
                ...
                if (!urlPattern.test(server.url)) {
                    throw new McpError(ErrorCode.InvalidParams, `Format d'URL Facebook invalide. L'URL doit être au format [...].facebook.com/XXXXXXXXXX/videos/XXXXXXXXXX/`);
                }
            }
        }
        try {
            const response = await axios.post(`${FB_API_BASE_URL}/${postId}/comments`, { message: JSON.stringify(episode) }, {
                params: {
                    access_token: this.accessToken,
                },
            });
    ...
    handleError(error) {
        ...
        if (error instanceof AxiosError) {
            ...
            if (error.response?.data?.error?.code === 3) {
                throw new McpError(ErrorCode.InternalError, `Erreur Facebook API: L'application n'a pas les permissions nécessaires pour effectuer cette opération. Permissions requises: 'publish_pages' et 'manage_pages'.`);
            }
            else if (error.response?.data?.error?.code === 190) {
                throw new McpError(ErrorCode.InternalError, `Erreur Facebook API: Le token d'accès a expiré ou est invalide. Veuillez régénérer un nouveau token d'accès.`);
            }
            else if (error.response?.data?.error?.code === 100) {
                throw new McpError(ErrorCode.InternalError, `Erreur Facebook API: Paramètre invalide ou manquant, vérifiez les identifiants fournis. Détail: ${error.response?.data?.error?.message}`);
            }
            else {
                throw new McpError(ErrorCode.InternalError, `Erreur Facebook API (${error.response?.data?.error?.code || "inconnu"}): ${error.response?.data?.error?.message || error.message}`);
            }
        }
