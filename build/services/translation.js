...
export class TranslationService {
    ...
    buildTranslationPrompt(text, targetLanguage, sourceLanguage, isAnimeTitle) {
  ...
  "detectedLanguage": "la langue source détectée (si non spécifiée)"
    ...
    parseGeminiResponse(geminiResponse) {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, "La réponse de l'API de traduction est invalide");
        }
    }
    handleError(error) {
        console.error("Translation API error:", error);
        if (error instanceof AxiosError) {
            throw new McpError(ErrorCode.InternalError, `Erreur lors de la traduction: ${error.response?.data?.error?.message || error.message}`);
        }
