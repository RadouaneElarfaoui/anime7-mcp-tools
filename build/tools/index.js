...
const facebookTools = [
    {
        ...
        handler: async (args) => {
            ...
            const result = await facebookService.getPagePosts(args.limit, args.after, args.before, args.since, args.until);
            return {
    ...
    },
    {
        ...
        handler: async (args) => {
            const facebookService = FacebookService.getInstance();
            const result = await facebookService.getPostComments(args.postId, args.limit, args.after);
            return {
                content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
            };
        },
    },
    {
        ...
        handler: async (args) => {
            if (!args.confirm) {
                throw new McpError(ErrorCode.InvalidParams, "Tu dois confirmer la suppression (confirm=true)");
            }
            const facebookService = FacebookService.getInstance();
